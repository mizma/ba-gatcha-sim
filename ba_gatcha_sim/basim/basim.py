...
def new_sim(
    target=2, cycles=10000, count=0, reset=False, rcount=2, use_mile=False, Verbose=0
):
    ...
    for _ in range(cycles):
        ...
        mileage = 0
        if use_mile:
            mileage_bonus = {70, 130, 150, 170, 270, 330, 350, 370}
        else:
            mileage_bonus = {}
        # run simulation for cycles times
        ...
        for pull in range(MAX_PULLS):
            pout(
                f"[{pull}] : pulls={pulls} p_cnt={p_cnt} tickets={tickets} utickets={tickets_used} mile={mileage}",
                Verbose,
                Level.DEBUG,
            )
            if tickets > 0:
                ...
                for _ in range(tickets):
                    ...
                    if new_pull(p_cnt, Verbose):
                        ...
                        p_cnt += 1
                pout(
                    f"[{pull}] : TICKET pulls={pulls} p_cnt={p_cnt} tickets={tickets} utickets={tickets_used} mile={mileage}",
                    Verbose,
                    Level.DEBUG,
                )
                if success:
...
def cmd(kwargs):
    ...
    if run_new and run_old:
        ...
        for i in range(MAX_PULLS + 1):
            print(
                f"{i},{(new_res['hist'][i] / cycles) * 100},{(new_res['cumul'][i] / cycles) * 100},{(old_res['hist'][i] / cycles) * 100},{(old_res['cumul'][i] / cycles) * 100}"
            )
    elif run_new:
        print("PULLS,NEW HIST,NEW CUMUL(%)")
        for i in range(MAX_PULLS + 1):
            print(
                f"{i},{(new_res['hist'][i] / cycles) * 100},{(new_res['cumul'][i] / cycles) * 100}"
            )
    elif run_old:
        print("PULLS,OLD HIST,OLD CUMUL(%)")
        for i in range(MAX_PULLS + 1):
            print(
                f"{i},{(old_res['hist'][i] / cycles) * 100},{(old_res['cumul'][i] / cycles) * 100}"
            )
    pass
